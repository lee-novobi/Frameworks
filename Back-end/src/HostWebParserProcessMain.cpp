...
#include "LogParser/HostWebLogParser.h"
...
int main(int argc, char* argv[])
{
	...
	if(CheckProc("HostWebParserProcess") != -1)
	{
	...
	printf ("Create HostWeb Parsing Process : %s !!\n", argv[0]);
	...
	if (ListCfgFile.is_open())
	{
		while(!ListCfgFile.eof())
		{
			...
			if (child_id) {
				...
				CHostWebLogParser *pHostWebParser = new CHostWebLogParser(strLine.c_str());
				pHostWebParser->ProcessParse();
				delete pHostWebParser;
			}
		}
