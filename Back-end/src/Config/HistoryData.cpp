 ...
 */
CHistoryData::CHistoryData(string strLogFile, string strInfoFile)
:CLogParserData(strLogFile, strInfoFile)
{
...
void CHistoryData::SetProcessPosition(string strProcessId, int iPosition)
{
	...
	Update(INFO, ssPosProperty.str(), ssPos.str());
}

void CHistoryData::SetLastDatetime(string strDatetime)
{
	Update(LAST, DATETIME, strDatetime);
}

string CHistoryData::GetLastDatetime()
{
	string line;
	line = ReadStringValue(LAST, DATETIME);
	if (line == "") 
	{
...
string CHistoryData::GetHistoryLogTail(string strProcessId)
{
	...
	line = ReadStringValue(INFO, strProcessTail);
	return line;
}
...
int CHistoryData::GetHistoryLogPosition(string strProcessId)
{
	...
	line = ReadStringValue(INFO, strProcessPosition);
	if (line != "") 
	{
		try{
