...
#include "Processor/DeleteTriggerProcess.h"
...
int main(int argc, char* argv[])
{
	...
	printf ("Create Event Parsing Process : %s !!\n", argv[0]);
	...
	if (ListCfgFile.is_open())
	{
		while(!ListCfgFile.eof())
		{
			...
			if (child_id) {
				...
				CDeleteTriggerProcess *pDeleteTriggerProcess = new CDeleteTriggerProcess(strLine.c_str());
				pDeleteTriggerProcess->ProcessDelete();
				delete pDeleteTriggerProcess;
			}
		}
