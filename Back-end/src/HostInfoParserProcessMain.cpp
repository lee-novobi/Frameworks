#include "LogParser/HostInfoLogParser.h"
	...
	if (ListCfgFile.is_open())
	{
		while(!ListCfgFile.eof())
		{
			...
			if (child_id) {
				...
				CHostInfoLogParser *pHostInfoParser = new CHostInfoLogParser(strLine.c_str());
				pHostInfoParser->ProcessParse();
				delete pHostInfoParser;
			}
		}
