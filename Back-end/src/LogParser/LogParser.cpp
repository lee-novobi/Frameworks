...
string LogParser::GetExpression(const char* chBuffer, int &nCurPosition, int iLength)
{
	...
	while(nCurPosition < iLength)
	{
		...
		{
			nCurPosition = nCurPosition + 1;
			if(chBuffer[nCurPosition] == '&' || chBuffer[nCurPosition] == '|' || chBuffer[nCurPosition] == ' ' 
					|| chBuffer[nCurPosition] == '(' || chBuffer[nCurPosition] == '<' || chBuffer[nCurPosition] == '>')
			{
...
string LogParser::GetTriggerDescription(const char* chBuffer, int &nCurPosition, int iLength)
{
		string strDescription;
		int iFlag1 = 0;
		int iFlag2 = 0;
...
void LogParser::GoToNewLine(const char* chBuffer, int &nCurPosition, int iLength)
{
	while (nCurPosition < iLength){
		if (chBuffer[nCurPosition] == '\n'){
			nCurPosition = nCurPosition + 1;
