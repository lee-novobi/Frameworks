...
void CUtilities::WriteErrorLog(CConfigFile* oConfig, const std::string& strErrorMsg)
{
	ofstream fErrorLog;
	string strErrorLog;
...
{	
	string strErrorLog;
	CConfigFile* oConfigReader = new CConfigFile("SyncConfig.ini");
	strErrorLog = oConfigReader->GetErrorLog();
	ofstream fErrorLog;
...
void CUtilities::WriteDataLog(const std::string& strDataInfo)
{
	string strInfoLog;
	string strDateTime;
	
	strDateTime = CUtilities::GetCurrTime("%Y%m%d");
	CConfigFile* oConfigReader = new CConfigFile("SyncConfig.ini");
	strInfoLog = oConfigReader->ReadStringValue("INFO_LOG_CONFIG", "InfoLogPrefix");
	strInfoLog += strDateTime + ".log";
	
	ofstream fInfoLog;
	fInfoLog.open(strInfoLog.c_str(),std::ofstream::app);
	fInfoLog << strDataInfo << "\n";
	fInfoLog.close();
	delete oConfigReader;
}
