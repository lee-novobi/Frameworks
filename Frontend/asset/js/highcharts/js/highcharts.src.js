...
PlotLineOrBand.prototype = {
	...
	render: function () {
		...
		// plot line
		if (width) {
			path = axis.getPlotLinePath(value, width, null, null, options);
			attribs = {
				stroke: color,
				'stroke-width': width
			};
		...
		if (optionsLabel && defined(optionsLabel.text) && path && path.length && axis.width > 0 && axis.height > 0) {
			...
			if(defined(options.categoryBand)){
				xs = [path[1], path[4], pick(path[7], path[1])];
				ys = [path[2], path[5], pick(path[8], path[2])];
			} else {
...
Axis.prototype = {
	 ...
	 */
	getPlotLinePath: function (value, lineWidth, old, force, options) {
		var axis = this,
			chart = axis.chart,
			axisLeft = axis.left,
			axisTop = axis.top,
		...
		if (isNaN(translatedValue)) { // no min or max
			...
			if(defined(options) && defined(options.categoryBand)){
				y1 = cHeight - axis.bottom;
				y2 = y1 + (options.categoryBandHeight || 20);
				x1 = x2 = x1-1;
			} else {
				y1 = axisTop;
	...
	getPlotBandPath: function (from, to, options) {
		...
		var toPath = this.getPlotLinePath(to, null, null, null, options),
			path = this.getPlotLinePath(from, null, null, null, options);
