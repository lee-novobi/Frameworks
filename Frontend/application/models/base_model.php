...
class Base_model extends CI_Model
{
	...
	// ------------------------------------------------------------------------------------------ //
 	...
 	function __construct($config = 'default')
 	{
		...
		//$this->mongo_config_write            = $this->config->item('write');
		$this->mongo_config_capacity_report  = $this->config->item('capacity');
		$this->mongo_config_dependency       = $this->config->item('dependency_map');
		$this->mongo_config_audit            = $this->config->item('audit');

		$this->tblActiveTable                = TBL_ACTIVE_TABLE;
		$this->tblDBCategory                 = TBL_DB_CATEGORY;
		$this->tblDepartments                = TBL_DEPARTMENTS;
		$this->tblEvents                     = TBL_EVENTS;
		$this->tblHistoryCPUUtil             = TBL_HISTORY_CPU_UTIL;
 		$this->tblHistoryServerLoad          = TBL_HISTORY_SERVER_LOAD;
 		$this->tblHistoryMemUsage            = TBL_HISTORY_MEM_USAGE;
 		$this->tblHistoryIOPrefix            = TBL_HISTORY_IO_PREFIX;
		$this->tblHistoryInt                 = TBL_HISTORY_INT;
		$this->tblHistoryFloat               = TBL_HISTORY_FLOAT;
		$this->tblHostViewDetail             = TBL_HOST_VIEW_DETAIL;
		$this->tblProducts                   = TBL_PRODUCTS;
		$this->tblOverviewHostsStatusSummary = TBL_OVERVIEW_HOSTS_STATUS_SUMMARY;
		$this->tblOverviewServicesStatus     = TBL_OVERVIEW_SERVICE_STATUS;
		$this->tblOverviewResUtil            = TBL_OVERVIEW_RES_UTIL;
		$this->tblOverviewHardwareStatus     = TBL_OVERVIEW_HARDWARE_STATUS;
		$this->tblOverviewHostsStatusDetail  = TBL_OVERVIEW_HOSTS_STATUS_DETAIL;
		$this->tblOverviewTop10CPU           = TBL_OVERVIEW_TOP10_CPU_UTIL;
		$this->tblOverviewTop10Mem           = TBL_OVERVIEW_TOP10_MEM_UTIL;
		$this->tblOverviewTop10ServerLoad    = TBL_OVERVIEW_TOP10_SERVER_LOAD_UTIL;
		$this->tblOverviewTop10Disk          = TBL_OVERVIEW_TOP10_DISK_UTIL;
		$this->tblRefCategory                = TBL_REF_CATEGORY;
		$this->tblRefSubcategoryItemKeys     = TBL_REF_SUBCATEGORY_ITEM_KEYS;
		$this->tblRefSubcategoryProcesses    = TBL_REF_SUBCATEGORY_PROCESSES;
		$this->tblSession                    = TBL_SESSION;
		$this->tblServerCategory             = TBL_SERVERS_CATEGORY;
		$this->tblServiceCategory            = TBL_SERVICES_CATEGORY;
		$this->tblAlert                      = TBL_ALERT;
		$this->tblWebServerCategory          = TBL_WEBSERVER_CATEGORY;
		$this->tblAppServerCategory          = TBL_APPSERVER_CATEGORY;
		$this->tblOtherServiceCategory       = TBL_OTHER_SERVICE_CATEGORY;
		$this->tblRefPartitionHistory		 = TBL_REF_PARTITION_HISTORY;

		$this->tblCapacityReportCPUUtil      = TBL_CAPACITY_REPORT_CPU_UTIL;
 		$this->tblCapacityReportMEMUtil      = TBL_CAPACITY_REPORT_MEM_UTIL;
 		$this->tblCapacityReportServerLoad   = TBL_CAPACITY_REPORT_SERVER_LOAD;
 		$this->tblCapacityReportDiskUsage    = TBL_CAPACITY_REPORT_DISK_USAGE;
 		$this->tblCapacityReportIORead       = TBL_CAPACITY_REPORT_IO_READ;
 		$this->tblCapacityReportIOWrite      = TBL_CAPACITY_REPORT_IO_WRITE;
 		$this->tblCapacityReportIOUtil       = TBL_CAPACITY_REPORT_IO_UTIL;
 		$this->tblUserProfile                = TBL_USER_PROFILE;
 		$this->tblHostNetStatService         = TBL_HOST_NETSTAT_SERVICE;
 		$this->tblCapacitySummaryReport      = TBL_CAPACITY_SUMMARY_REPORT;
 		$this->tblAuditSession               = TBL_AUDIT_SESSION;
 		$this->tblAuditVictim                = TBL_AUDIT_VICTIM;
		$this->tblAuditRefServices           = TBL_AUDIT_REF_SERVICES;
 		$this->tblAuditResult                = TBL_AUDIT_RESULT;
 		$this->tblApplicationDependency      = TBL_APPLICATION_DEPENDENCY;
	
		$this->GetActiveTables();

		$this->eventStatusMap = array(
			EVENT_STATUS_OK	          => 'OK',
			EVENT_STATUS_PROBLEM      => 'PROBLEM'
		);

		$this->hostStatusMap = array(
			HOST_STATUS_MONITORED	  => 'Monitored',
			HOST_STATUS_NOT_MONITORED => 'Not Monitored',
			HOST_STATUS_TEMPLATE      => 'Host Template',
			HOST_STATUS_PROXY         => 'Proxy'
		);

		$this->hostAvailabilityMap = array(
			HOST_STATUS_UNKNOWN	      => 'Unknown',
			HOST_STATUS_UP            => 'Up',
			HOST_STATUS_DOWN          => 'Down'
		);
		$this->eventAckMap = array(
			ACK_STATUS_YES            => 'Yes',
			ACK_STATUS_NO             => 'No'
		);
		...
		//$this->isFullRight = $this->userType == ZABBIX_SUPPER_ADMIN
		//                     || $this->userType == ZABBIX_SDK_SUPPER_ADMIN;

		$this->userRightGroup = $this->ListGroupByUserId($userId);

		if(!isset($_SESSION['userRightHost']))
			$this->userRightHost = array();
		else
			$this->userRightHost = $_SESSION['userRightHost'];
	}
	// ------------------------------------------------------------------------------------------ //
	function GetActiveTables()
	{
		...
		$this->tblAcknowledges = $arr_activeTables[TBL_ACKNOWLEDGES]['active'];
		$this->tblAlert        = $arr_activeTables[TBL_ALERTS]['active'];
		$this->tblApplications = $arr_activeTables[TBL_APPLICATIONS]['active'];
		$this->tblGroups       = $arr_activeTables[TBL_GROUPS]['active'];
		$this->tblHosts        = $arr_activeTables[TBL_HOSTS]['active'];
		$this->tblHostsGroups  = $arr_activeTables[TBL_HOSTS_GROUPS]['active'];
		$this->tblItems        = $arr_activeTables[TBL_ITEMS]['active'];
		$this->tblItemsApplications = $arr_activeTables[TBL_ITEMS_APPLICATION]['active'];
		$this->tblRights       = $arr_activeTables[TBL_RIGHTS]['active'];
		$this->tblTriggers     = $arr_activeTables[TBL_TRIGGER]['active'];
		$this->tblUsers        = $arr_activeTables[TBL_USERS]['active'];
		$this->tblUsersGroups  = $arr_activeTables[TBL_USERS_GROUPS]['active'];
		$this->tblUsrGrp       = $arr_activeTables[TBL_USRGROUP]['active'];

		$this->tblDBCategory                 = $arr_activeTables[TBL_DB_CATEGORY]['active'];
		$this->tblHostViewDetail             = $arr_activeTables[TBL_HOST_VIEW_DETAIL]['active'];
		$this->tblOverviewHostsStatusSummary = $arr_activeTables[TBL_OVERVIEW_HOSTS_STATUS_SUMMARY]['active'];
		$this->tblOverviewServicesStatus     = $arr_activeTables[TBL_OVERVIEW_SERVICE_STATUS]['active'];
		$this->tblOverviewResUtil            = $arr_activeTables[TBL_OVERVIEW_RES_UTIL]['active'];
		$this->tblOverviewTop10CPU           = $arr_activeTables[TBL_OVERVIEW_TOP10_CPU_UTIL]['active'];
		$this->tblOverviewTop10Mem           = $arr_activeTables[TBL_OVERVIEW_TOP10_MEM_UTIL]['active'];
		$this->tblOverviewTop10ServerLoad    = $arr_activeTables[TBL_OVERVIEW_TOP10_SERVER_LOAD_UTIL]['active'];
		$this->tblOverviewTop10Disk          = $arr_activeTables[TBL_OVERVIEW_TOP10_DISK_UTIL]['active'];
        $this->tblOverviewHostsStatusDetail  = $arr_activeTables[TBL_OVERVIEW_HOSTS_STATUS_DETAIL]['active'];
        $this->tblServerCategory             = $arr_activeTables[TBL_SERVERS_CATEGORY]['active'];
        $this->tblServiceCategory            = $arr_activeTables[TBL_SERVICES_CATEGORY]['active'];
        $this->tblAlert                      = $arr_activeTables[TBL_ALERT]['active'];
        $this->tblWebServerCategory          = $arr_activeTables[TBL_WEBSERVER_CATEGORY]['active'];
        $this->tblAppServerCategory          = $arr_activeTables[TBL_APPSERVER_CATEGORY]['active'];
        $this->tblOtherServiceCategory       = $arr_activeTables[TBL_OTHER_SERVICE_CATEGORY]['active'];
		$this->tblTopServiceReport			 = $arr_activeTables[TBL_TOP_SERVICE_REPORT]['active'];
		$this->tblResourceUsageReport		 = $arr_activeTables[TBL_RESOURCE_USAGE_REPORT]['active'];
	}
	// ------------------------------------------------------------------------------------------ //
