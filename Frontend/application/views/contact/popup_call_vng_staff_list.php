...
<body style="background-color: white; ">
	<div id="tblActionCallWrapper">
		<div id="divPhoneRinging" class="t-center" style="padding-top: 30px;">
			...
			<p style="font-family:verdana;"><label id="lblCall">Calling </label><b><?php echo htmlentities($arrUser['fullname'], ENT_QUOTES, 'UTF-8'); ?></b> - <b id="bMobile"><?php echo @$arrUser['cellphone'][0]; ?></b>.........<p>
		</div>
		<div class="t-center" style="padding-top: 30px;">
			<a id="btnSuccess" class="peter-river-flat-button hand-pointer">Call Success</a>
			<a id="btnRecall" class="emerald-flat-button hand-pointer" phone_num="<?php echo @$arrUser['cellphone'][0]; ?>">Recall</a>
			<a id="btnFail" class="alizarin-flat-button hand-pointer">Call Fail & SMS</a>
		</div>
	</div>
	<?php if(!empty($arrUser)) { ?>
	<?php if(count($arrUser['cellphone']) > 1) { ?>
	<br />
	<div style="margin: auto; width: 880px; padding-top: 10px; background-color: white;" class="t-center">
	<table id="tblCallUser" width="100%" cellpadding="0" cellspacing="0" class="list">
		<thead>
			<tr class="table-title">
				<th colspan="5"><p class="drag" style="margin-top: 0;">User <?php echo htmlentities($arrUser['fullname'], ENT_QUOTES, 'UTF-8'); ?>'s phone numbers</p></th>
			</tr>
			<tr>
		...
		<?php foreach($arrUser['cellphone'] AS $idx=>$rowMobile) {?>
			<tr>
...
$(document).ready(function() {
	...
	{
		var iUserId = <?php echo $arrUser['id']; ?>;
		
		$.ajax({
			type: 'POST',
			url: '<?php echo $base_url; ?>contact/contact/call_staff_success',
			data: {
				iUserId:iUserId,
				incident_id:inc_id,
				alert_id:alert_id,
			...
			success: function(result) {
				...
				<?php if(count($arrUser['cellphone']) == 1) { ?>
					parent.$.fancybox.close();
				<?php } elseif (count($arrUser['cellphone']) > 1) { ?>
	...
	$('#btnFail').click(function()
	{
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo $base_url; ?>contact/contact/call_staff_fail',
			data: {
				iUserId:iUserId,
				incident_id:inc_id,
				alert_id:alert_id,
				alert_msg:alert_msg,
				time_alert:time_alert,
				user_in:'<?php echo HR_VNG;?>',
				change_id:change_id
			},
			success: function(result) {
				var url = base_url + 'contact/contact/load_popup_sms_vng_staff_list?userid=' + iUserId + '&incident_id=' + inc_id + '&alert_id=' + alert_id + '&alert_msg=' + encodeURIComponent(alert_msg) + '&time_alert=' + time_alert + '&sms_identifier=1' + '&change_id=' + change_id;
				<?php if(count($arrUser['cellphone']) == 1) { ?>
					// parent.$.fancybox.close();
	...
	$('#btnRecall').click(function() {
		...
		$.ajax({
			...
			data: {
				strMobile:strMobile,
				user_name:'<?php echo $arrUser['fullname'];?>',
				ref_id:'<?php echo $arrUser['id'];?>',
				incident_id:inc_id,
				alert_id:alert_id,
				alert_msg:alert_msg,
				time_alert:time_alert,
