...
$(document).ready(function () {
	...
	$("#send").click(function(){
		...
		$.ajax({
		    ...
		    data:{
		    	userid:<?php echo $arrOneUser["userid"] ?>, 
		    	message:message, 
		    	user_name:'<?php echo $arrOneUser['full_name'];?>', 
...
<div style="background-color: white;">
    <table id="tblSendSMS" cellpadding="10" cellspacing="0" width="100%" border="1" style="border-color: lightgrey; border-style: solid; border-width: 0; padding: 5px;">
    	...
    	<tr>
    		<td>Send to</td>
    		<td colspan="2">
				<?php $output = htmlentities($arrOneUser['full_name'], ENT_QUOTES, 'UTF-8');
                    foreach($arrOneUser['mobile'] as $idx=>$mob) {
                        if($idx == 0) {
